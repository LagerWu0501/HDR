...
def recover(images, log_delta_t, image_num, pixel_num, selected_pixels, g_constraint = 128, Lambda=100, max_iter=1):
    ...
    for t in range(max_iter):
        ...
        for r in range(images.shape[0]):
            ...
            self_vector_space = np.zeros((images[0].shape[0], images[0].shape[1], dim))
            self_vector_space[:, :, :3] = images[r]

            for x in range(images[0].shape[0]):
                for y in range(images[0].shape[1]):
                    self_vector_space[x, y, 3] = x  
                    self_vector_space[x, y, 4] = y  

            for s in range(images.shape[0]):
                w_average = np.average(w_map[s], axis=2)  

                vector_space = np.zeros((images[0].shape[0], images[0].shape[1], dim))
                vector_space[:, :, :3] = images[s]
                ...
                sum_w_pqs[:-1, :-1, 0] += w_average[1:, 1:]
                sum_w_pqs[:-1, :-1, 1] += w_average[1:, 1:]
                sum_w_pqs[:-1, :-1, 2] += w_average[1:, 1:]
                sum_w_pqs[1:, 1:, 0] += w_average[:-1, :-1]
                sum_w_pqs[1:, 1:, 1] += w_average[:-1, :-1]
                sum_w_pqs[1:, 1:, 2] += w_average[:-1, :-1]
                ...
                # up and down
                vector = ((self_vector_space[:-1, :] - vector_space[1:, :])*(self_vector_space[:-1, :] - vector_space[1:, :])).sum(axis=2)
                ...
                P_map[:, :-1, 2] += w_average[:, 1:] * H_d * (2 * np.pi)**(-dim / 2) * np.exp(-1/2 * vector)
                # upper left and lower right
                vector = ((self_vector_space[:-1, :-1] - vector_space[1:, 1:]) * (self_vector_space[:-1, :-1] - vector_space[1:, 1:])).sum(axis=2)
    ...
    if len(parameter) == 1:
        ...
        if (len(parameter) == 2):
            ...
            if (len(parameter) == 10):
                ...
                for path in paths:
                    # a = cv2.imread(path)
                    # images.append(cv2.resize(a, (int(a.shape[0] * 0.1), int(a.shape[1] * 0.1))))
                    images.append(cv2.imread(path))
                images = np.array(images, dtype='int16')
